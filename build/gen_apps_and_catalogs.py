...
def fetch_catalog_hierarchy(catalog_id, hierarchy=None):
    ...
    # 如果存在父目录，递归调用
    if hasattr(catalog_entry, 'parent_catalog'):
...
def generate_allcatalogs_md(products):
    ...
    for product in products:
        if hasattr(product, 'catalogs'):
            for catalog_id in product.catalogs:
                # 获取目录层级
                hierarchy = fetch_catalog_hierarchy(catalog_id.sys.id)
                if len(hierarchy) > 1:
                    # 构建 Markdown 行
                    line = f"## {hierarchy[-2]}\n\n- [{hierarchy[-1]}](https://www.websoft9.com/apps/{product.key})"
                    lines.append(line)
    return '\n\n'.join(lines)
...
# 转换为 Markdown 格式
markdown_content_apps = '\n'.join(f"- {trademark}" for trademark in trademarks)
