...
![](https://libs.websoft9.com/Websoft9/DocsPicture/zh/runtime/runtime-web-websoft9.png)
...
1. 登录 Websoft9 控制台，打开："应用商店" 的 **运行环境** 类目
   ![](https://libs.websoft9.com/Websoft9/DocsPicture/zh/runtime/runtime-catalog-websoft9.png)

2. 任选一个程序语言，例如：Python，点击**安装**
...
2. 修改编排文件的 *src/cmd.sh* 脚本（它包含范例），**插入个性化命令**或**去掉范例的注释**，重建应用后生效
  ![](https://libs.websoft9.com/Websoft9/DocsPicture/zh/runtime/runtime-composeedit-cmd-websoft9.png)

   建议插入到 cmd.sh 的命令主要包括：

   > 查看 cmd.sh 范例：[PHP](https://websoft9.github.io/docker-library/apps/php/src/cmd.sh), [Python](https://websoft9.github.io/docker-library/apps/python/src/cmd.sh), [Ruby](https://websoft9.github.io/docker-library/apps/ruby/src/cmd.sh), [Go](https://websoft9.github.io/docker-library/apps/go/src/cmd.sh), [Node.js](https://websoft9.github.io/docker-library/apps/nodejs/src/cmd.sh), [.NET](https://websoft9.github.io/docker-library/apps/dotnet/src/cmd.sh), [OpenJDK](https://websoft9.github.io/docker-library/apps/openjdk/src/cmd.sh), [Tomcat](https://websoft9.github.io/docker-library/apps/tomcat/src/cmd.sh), [Tomee](https://websoft9.github.io/docker-library/apps/tomee/src/cmd.sh), [Maven](https://websoft9.github.io/docker-library/apps/maven/src/cmd.sh)
...
如果用户在开发测试中，不想通过 cmd.sh 带入应用的启动命令，也可以参考如下的命令在后台中运行应用程序：

1. 进入 Python 容器的命令模式

2. 以后台方式运行启动命令（将运行程序的命令插入 `nohup  Your CMD here  > output.log 2>&1 &` 中间）

   ```
   nohup python manage.py runserver 0.0.0.0:8080 > output.log 2>&1 &
   ```

3. 程序即可在后台运行，同时通过 output.log 查看日志
