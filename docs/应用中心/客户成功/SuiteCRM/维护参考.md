...
SuiteCRM 预装包包含 SuiteCRM（LAMP） 运行所需一序列支撑软件（简称为“组件”），下面列出主要组件名称、安装路径、配置文件地址、端口、版本等重要的信息。
...
2. 通过 phpMyAdmin 导出 SuiteCRM 数据库
...
此处收集使用 SuiteCRM 过程中最常见的故障，供您参考
