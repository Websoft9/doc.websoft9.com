...
不同的云平台的安全组操作方式略有差异，请参考：[《云服务器操作》](./user/vm)  
...
* Windows服务器：直接 [远程桌面](./user/vm#connectwindows) 登录后，通过拷贝和粘贴的方式管理文件
* Linux服务器：请使用 [SFTP 连接服务器](./user/vm#connectlinux) 后，通过可视化界面管理文件
...
如果您需要更换（或重置）镜像，请参考：[《云服务器操作》](./user/vm)  
