...
3. 在服务器中查看 MySQL/MariaDB 的 **[默认账号和密码](./setup/credentials#getpw)**  
4. 若想用域名访问  MySQL/MariaDB，务必先完成 **[域名五步设置](./dns#domain)** 过程
...
3. 使用 root [账号密码](./setup/credentials#getpw) 登录 MySQL 服务端
    ~~~
    #假设初始密码是：123456
    mysql -uroot –p123456
    ~~~
...
2. 输入数据库用户名和密码([不知道密码？](./setup/credentials#getpw))
