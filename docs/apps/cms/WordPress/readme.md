...
3. 在服务器中查看 WordPress 的 **[默认账号和密码](./setup/credentials#getpw)**  
...
3. 系统进入数据库连接信息安装项，请填写数据库连接信息（[不知道账号密码？](./setup/credentials#getpw)） 
...
9. 使用主题，参考：[WordPress 使用 Avada 主题9步建站](./wordpress/solution#wordpress-avada)
...
[WordPress 使用 Avada 主题9步建站](./wordpress/solution#wordpress-avada)
...
6. SMTP配置成功后，所有的WordPress后台邮件发送就会使用这个配置
...
使用主题，参考：[WordPress 使用 Avada 主题9步建站](./wordpress/solution#wordpress-avada)
...
参考：[WordPress 与 Discuz 集成](./wordpress/solution#wordpress-discuz)

### 使用对象存储

参考：[WordPress 使用对象存储](./wordpress/solution#wordpress-oss)
...
| 端口号 | 用途                                          | 必要性 |
| ------ | --------------------------------------------- | ------ |
| 8080   | WordPress 原始端口，已通过 Nginx 转发到 80 端口 | 可选   |
