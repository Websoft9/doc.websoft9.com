...
Dolibarr 预装包包含 Dolibarr 运行所需一序列支撑软件（简称为“组件”），下面列出主要组件名称、安装路径、配置文件地址、端口、版本等重要的信息。
...
2. 通过 phpMyAdmin 导出 Dolibarr 数据库
...
此处收集使用 Dolibarr 过程中最常见的故障，供您参考
...
1. 编辑 [MySQL 配置文件](#mysql)，注释掉 binlog 日志
  ~~~
  #log-bin=mysql-bin  
  ~~~
2. 重启mysql
...
通过修改 [Apache 虚拟主机配置文件](#apache) 中相关路径参数
