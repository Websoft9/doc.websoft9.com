...
3. 在服务器中查看 Graylog 的 **[默认账号和密码](./setup/credentials#getpw)**
4. 若想用域名访问 Graylog，务必先完成**[域名五步设置](./dns#domain)** 过程
...
2. 输入账号密码后，登入到 Graylog 控制台 ([不知道密码?](./setup/credentials#getpw))  
...
只需在参数名称前加上前缀 GRAYLOG\_ 并将其全部大写。
