...
1. 使用本地电脑的浏览器访问网址：*http://域名* 或 *http://Internet IP*, 就进入登录页面
...
1. 管理控制台获取 SMTP 相关参数
   ...
   SMTP password: #wwBJ8
   ```
3. 编辑 /usr/share/grafana/conf/defaults.ini 中的 SMTP选项，填写自己的 SMTP 项
   ```
   #################################### SMTP / Emailing #####################
   [smtp]
   enabled = false
...
4. 重启服务
   ...
   ```
5. 登录 Grafana 控制台，打开：【Alerting】>【Alert Rules】，新建一个【Email】通知渠道
   ![Grafana SMTP](https://libs.websoft9.com/Websoft9/DocsPicture/en/grafana/grafana-sendmails-websoft9.png)
6. 点击【Send Test】

## 参数

**[通用参数表](./setup/parameter)** 中可查看 Nginx, Docker, SQLite 等 Grafana 应用中包含的基础架构组件路径、版本、端口等参数。 
...
```shell
sudo docker  start | stop | restart | status grafana-server
```
...
[Grafana API](https://grafana.com/docs/grafana/latest/http_api) 采用REST API 2.0规范。
