...
3. 在服务器中查看 AWX 的 **[默认管理员账号和密码](./setup/credentials)**  
...
3. 输入用户名和密码[（不知道密码？）](./setup/credentials)，登录到 AWX 后台管理界面
...
AWX 应用中包含 Nginx, Docker, PostgreSQL 等组件，可通过 **[通用参数表](./setup/parameter)** 查看路径、服务、端口等参数。 
