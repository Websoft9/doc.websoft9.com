...
3. 在服务器中查看 Redmine 的 **[默认账号和密码](./setup/credentials#getpw)**  
4. 若想用域名访问  Redmine **[域名五步设置](./dns#domain)** 过程
...
2. 点击【登录】，进入系统（[不知道账号密码？](./setup/credentials#getpw)）
...
1. 在QQ邮箱管理控制台获取 [SMTP](./automation/smtp) 相关参数
...
**[通用参数表](./setup/parameter)** 中可查看 Nginx, Apache, Docker, MySQL 等 Redmine 应用中包含的基础架构组件路径、版本、端口等参数。 
